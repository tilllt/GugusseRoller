...
if [ $dirExists == NO ]; then
    ...
    ncftpls -u$ftpuser -p$ftppassword ftp://$ftpserver/$ftppathprefix/$2/*.dng > /dev/shm/filesOnServer.txt
    lastFile=`cat /dev/shm/filesOnServer.txt | sort | tail -n 1`
    echo lastfile=$lastFile	
    if [ -z "$lastFile" ]; then
	echo "The directory existed but we couldn't find a dng in it"
	echo "therefore we'll start with 0, press Enter if you agree"
	echo "or press Ctrl-C to cancel"
	read
	export startNumber=0
    elif [ "$lastFile" == "00000.dng" ]; then
	export startNumber=1
    else	
	lastFileNum=${lastFile%.dng}
