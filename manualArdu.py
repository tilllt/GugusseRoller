...
previewW=1024
previewH=768
global cfg,handle,running,Width,Heigth,save_flag,color_mode,save_raw,zoom,quadra,aperture,grid
running = True
save_flag = False
save_raw = False
zoom=0
grid=True
quadra=[0x1E,0x1D,0x1C,0x1B]
aperture=0x20
cfg = {}
handle = {}
...
def readImage_thread():
    global handle,running,Width,Height,save_flag,cfg,color_mode,save_raw,zoom,grid
    ...
    while running: 
        ...
        if ArducamSDK.Py_ArduCam_availableImage(handle) > 0:		
            ...
            if zoom > 5:
                ...
                image=image[int(Height/2-20):int(Height/2+20),int(Width/2-20):int(Width/2+20)]
            elif zoom != 0:
                startx=int(Width*zoom/10)
                starty=int(Height*zoom/10)
                endx=int(Width-startx)
                endy=int(Height-starty)
                #print("\r")
            ...
            if grid:
                for i in range (1,10):
                    x=int(previewW*i/10)
                    for y in range(0,previewH-1):
                        image[y,x]=0
                        image[y,x-1]=255
                for i in range (1,8):
                    y=int(previewH*i/8)
                    for x in range(0,previewW-1):
                        image[y,x]=0
                        image[y-1,x]=255
